...
Beverages::Beverages(
    ...
    std::string _Roast_level,
    std::string _sweetness,
    std::string _liquid_content_description, 
    std::string _primary_supplement_type, 
    std::string _Body_description,

    std::initializer_list <std::string> _Occassion,

    double Caffeine_content_value,
    std::initializer_list <std::string> Caffeine_content_unit_and_string,
...
:   Bvrgs_Product_Name(_product_name),
    Bvrgs_Brand_Name(_brand_name),
    ...
    Bvrgs_Roast_level(_Roast_level),
    Bvrgs_Sweetness(_sweetness),
    Bvrgs_Liquid_Content_Description(_liquid_content_description),
    Bvrgs_Primary_Supplement_Type(_primary_supplement_type),
    Bvrgs_Body_Description(_Body_description),

    Bvrgs_Occasion(_Occassion),

    Bvrgs_Caffeine_content(
        Caffeine_content_value,
        Caffeine_content_unit_and_string),

    Bvrgs_Is_Discontinued_by_manufacturer(_is_Discontinued_by_manufacturer),

    Bvrgs_Additional_Info(
        _ASIN,
...
void Beverages::printing_vector_string(std::vector <std::string> string_vector_object) const{
    int j = 0;
...
std::ostream& operator<<(std::ostream& os, const Beverages& Bvrgs_object){
    os << "PRODUCT DETIALS : " << std::endl << std::endl

    << "Product Name : " << Bvrgs_object.Bvrgs_Product_Name << std::endl
    << "Brand Name : " << Bvrgs_object.Bvrgs_Brand_Name << std::endl << std::endl;

    std::vector<Variants>::const_iterator Iter = 
    Bvrgs_object.Bvrgs_Variants_Object.begin();

    while(Iter != Bvrgs_object.Bvrgs_Variants_Object.end())
    {
    ...
    Bvrgs_object.Show_General_Groceries_Details();

    os 
    << "Roast level : " << Bvrgs_object.Bvrgs_Roast_level << std::endl
    << "Sweetness : " << Bvrgs_object.Bvrgs_Sweetness << std::endl
    << "Liquid Content Description : " << Bvrgs_object.Bvrgs_Liquid_Content_Description << std::endl
    << "Primary Supplement type : " << Bvrgs_object.Bvrgs_Primary_Supplement_Type << std::endl
    << "Body Description : " << Bvrgs_object.Bvrgs_Body_Description << std::endl;

    Bvrgs_object.printing_vector_string(Bvrgs_object.Bvrgs_Occasion);

    os 
    << "Caffeine Content : " << Bvrgs_object.Bvrgs_Caffeine_content << std::endl

    << "Additional Information : " << std::endl << std::endl
    << "Is Discontinued By manufacturer : " << Bvrgs_object.Bvrgs_Is_Discontinued_by_manufacturer << std::endl
    << Bvrgs_object.Bvrgs_Additional_Info;
