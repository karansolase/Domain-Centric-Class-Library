...
#ifndef _BEVERAGES_HPP
#define _BEVERAGES_HPP
...
class Beverages : protected General_Groceries_Details{

    friend std::ostream& operator<<(std::ostream& os, const Beverages& Bvrgs_object);

    private :
    std::string Bvrgs_Product_Name;
    std::string Bvrgs_Brand_Name;

    std::vector <Variants> Bvrgs_Variants_Object;
 
    std::string Bvrgs_Roast_level;
    std::string Bvrgs_Sweetness;
    std::string Bvrgs_Liquid_Content_Description; 
    std::string Bvrgs_Primary_Supplement_Type; 
    std::string Bvrgs_Body_Description;

    std::vector <std::string> Bvrgs_Occasion;

    Measurement_and_unit Bvrgs_Caffeine_content; // 0.03 grams || Low Caffeine 

    std::string Bvrgs_Is_Discontinued_by_manufacturer; 
    Additional_Info Bvrgs_Additional_Info;
