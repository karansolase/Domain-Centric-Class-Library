...
std::ostream& operator<<(std::ostream& os, const Date& Date_obj){
    ...
    else if(Date_obj.Num_for_constructor == 7)
    {
        os
        << Date_obj.m_Measurement_and_unit_Object;
    }
...
Date::Date(
    double value,
    std::initializer_list <std::string> unit_and_string)  :

    m_Measurement_and_unit_Object(value, unit_and_string)
{
