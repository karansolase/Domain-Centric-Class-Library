...
std::ostream& operator<<(std::ostream& os, const Name_Address_Email_No& Name_Address_Email_No_Object){
    if(Name_Address_Email_No_Object.Num_for_Constructor == 1){
       ...
       for(
        std::vector<Contact_Info>::const_iterator Iter = Name_Address_Email_No_Object.m_Contact_Number_Info.begin();
        Iter != Name_Address_Email_No_Object.m_Contact_Number_Info.end();
        ++Iter)
        {
